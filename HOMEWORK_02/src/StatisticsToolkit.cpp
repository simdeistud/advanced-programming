...
double StatisticsToolkit::variance(const std::vector<double>& data) {
    ...
    return std::sqrt(std::transform_reduce(data.begin(), data.end(), std::plus, [](auto x) (return std::pow(x - m, 2))) / data.size())
}
...
std::size_t StatisticsToolkit::count(const std::vector<double>& data, double& element) {
    return std::count(data.begin(), data.end(), element);
}

std::unordered_map StatisticsToolkit::frequency_map(const std::vector<double>& data) {
    std::unordered_map<double, std::size_t> freq_map = std::unordered_map();
    for (auto e : data) {
